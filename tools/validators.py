...
CONC = int(os.getenv("HTTP_CONCURRENCY", 20))
UA = os.getenv("USER_AGENT", "urlx/0.1")
ALLOW = {200,204,301,302,401,403,405}

async def _check_one(client, url):
    try:
        ...
        return ok, r.status_code, clen
    except Exception:
        return False, 0, 0

async def validate_urls(urls):
    results = {}
    limits = anyio.CapacityLimiter(CONC)
    async with httpx.AsyncClient(timeout=TIMEOUT, headers={"User-Agent": UA}) as client:
        async def worker(u):
            async with limits:
                ok, sc, clen = await _check_one(client, u)
                results[u] = (ok, sc, clen)
